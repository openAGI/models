...
dataflow = Dataflow(
    dataset, num_readers=num_readers, shuffle=True, min_queue_examples=100, capacity=200)

# [image, label] = dataflow.get(['image', 'label'], [32, 32, 3])
image_batch, label_batch = dataflow.get_batch(
    32,
    tf.convert_to_tensor([0.1 for _ in xrange(0, 10)]), [32, 32, 3],
    init_probs=tf.convert_to_tensor([0.1 for _ in xrange(0, 10)]),
    threads_per_queue=4)
images, labels = distorted_inputsv2(
    dataflow, [32, 32, 3], [28, 28],
    target_probs=tf.convert_to_tensor([0.1 for _ in range(0, 10)]),
    init_probs=tf.convert_to_tensor([0.1 for _ in xrange(0, 10)]),
    batch_size=32,
