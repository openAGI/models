...
def dense_block(inputs, num_filters, num_layers=4, drop_p=None, growth_rate=32, block_name='dense', **kwargs):
    cum_inputs = inputs
    for i in range(0, num_layers):
        x = conv_block(cum_inputs, num_filters,
                       block_name=block_name + '_' + str(i), drop_p=drop_p, **kwargs)
...
def model(inputs, is_training, reuse, input_size=image_size[0], drop_p_conv=0.0, drop_p_trans=0.0, n_filters=64, n_layers=[1, 2, 2, 3], num_classes=5):
    ...
    for block_idx in range(3):
        x, n_filters = dense_block(
            x, n_filters, num_layers=n_layers[block_idx], drop_p=drop_p_conv, block_name='dense_' + str(block_idx), **conv_args)
        x = trans_block(x, n_filters, drop_p=drop_p_trans,
                        block_name='trans_' + str(block_idx), **conv_args)
