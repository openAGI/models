...
def conv2d_same(inputs, num_outputs, kernel_size, stride, rate=1, name=None, **kwargs):
    ...
    """
    if stride == 1:
        return dilated_conv2d(inputs, num_outputs, filter_size=(kernel_size, kernel_size), dilation=rate,
        ...
        return dilated_conv2d(inputs, num_outputs, filter_size=kernel_size, stride=stride,
...
def bottleneck_se(inputs, depth, depth_bottleneck, stride, rate=1, name=None, **kwargs):
    ...
    with tf.variable_scope(name, 'bottleneck_se', [inputs]):
        ...
        if depth * 4 == depth_in:
            ...
            shortcut = conv2d(preact, depth * 4, is_training, reuse, filter_size=(1, 1), stride=(
                stride, stride), batch_norm=None, activation=None, name='shortcut')

        residual = conv2d(preact, depth, filter_size=(1, 1), stride=(1, 1),
                          name='conv1', **kwargs)
        residual = conv2d(residual, depth, filter_size=(3, 3), stride=(stride, stride),
                          name='conv2', **kwargs)
        residual = conv2d(residual, depth * 4, is_training, reuse, filter_size=(1, 1), stride=(1, 1),
                          batch_norm=None, activation=None, name='conv3')

        squeeze = global_avg_pool(residual, name='se_global_avg_pool')
        squeeze = fully_connected(
            squeeze, depth // 4, is_training, reuse, name='fc1')
        squeeze = fully_connected(
            squeeze, depth * 4, is_training, reuse, name='fc2')
