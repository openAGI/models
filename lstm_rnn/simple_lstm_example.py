...
class GenerateData(object):
  ...
  def __init__(self, num_examples=2**20, seq_length=20, is_shuffle=True, num_test_examples=10000):
    random_data = [
        '{0:0{seq_length}b}'.format(i, seq_length=seq_length) for i in range(num_examples)
    ]
    if is_shuffle:
      shuffle(random_data)
  ...
  def next(self, batch_size, train=True):
    if self.batch_id == len(self.data):
      self.batch_id = 0
    batch_data = (self.traindata[self.batch_id:min(self.batch_id + batch_size, len(self.data))])
    batch_labels = (self.trainlabels[self.batch_id:min(self.batch_id + batch_size, len(self.data))])
...
def train(data,
  ...
  with tf.Session() as sess:
    ...
    for step in range(n_iters_per_epoch * epoch):
      ...
      predict_error = sess.run(error, {input_data: testdata, target: testlabels})
      print('Batch {:2d} error {:3.1f}%'.format(step + 1, 100 * predict_error))
