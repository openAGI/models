...
def load_data():
    ...
    vocab = ['PAD', 'EOW', 'UNK'] + [char for char,
                                     count in char_counts.most_common(100)]
...
def model(vocab_size, batch_size):
    ...
    encoder_inputs_length = tf.placeholder(
        shape=[None, ], dtype=tf.int32, name='encoder_input_length')
    decoder_targets = tf.placeholder(
        shape=[None, None], dtype=tf.int32, name='decoder_targets')
    embeddings = tf.Variable(tf.random_uniform(
        [vocab_size, input_embedding_size], -1., 1.), dtype=tf.float32)
    ...
    ((encoder_fw_outputs, encoder_bw_outputs), (encoder_fw_final_state, encoder_bw_final_state)) = tf.nn.bidirectional_dynamic_rnn(
        cell_fw=encoder_cell, cell_bw=encoder_cell, inputs=encoder_inputs_embedded, sequence_length=encoder_inputs_length, dtype=tf.float32, time_major=True)
    ...
    eow_step_embedded, pad_step_embedded = decoder_input(
    ...
    def loop_fn(time, previous_output, previous_state, previous_loop_state):
        if previous_state is None:
            ...
            return loop_fn_transition(decoder_lengths, time, previous_output, previous_state, previous_loop_state, pad_step_embedded, embeddings, W, b)
...
def loop_fn_transition(decoder_lengths, time, previous_output, previous_state, previous_loop_state, pad_step_embedded, embeddings, W, b):
    def get_next_input():
...
def get_feed_dict(encoder_inputs, encoder_inputs_length, decoder_targets, words, vocab, lengths=None):
    encoder_inputs_, encoder_input_lengths_ = create_batch(
        [word_to_ids(vocab, seq) for seq in words])
    ...
    with tf.Session() as sess:
        ...
        for repeats in range(3):
            for batch in range(0, len(words) // batch_size):
                ...
                fd = get_feed_dict(
                    encoder_inputs, encoder_inputs_length, decoder_targets, words[i:j], vocab)
                ...
                if batch == 0 or batch % batches_in_epoch == 0:
                    ...
                    for i, (inp, pred) in enumerate(zip(fd[encoder_inputs].T, predict_.T)):
                        inp_txt = ''.join([vocab_inv.get(item, '?')
                                           for item in inp if item >= 3])
                        pred_txt = ''.join([vocab_inv.get(item, '?')
                                            for item in pred if item >= 3])
