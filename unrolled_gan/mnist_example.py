...
def generator(z, is_training, reuse, n_hid=500, isize=28 * 28, use_bn=False):
  ...
  if use_bn:
    fc = tf.nn.relu(batch_norm(fc, is_training=is_training, reuse=reuse, name='fc1/bn'))
  else:
    fc = tf.nn.relu(hid, name='fc1/prelu')
  fc = dense(fc, n_hid, is_training, reuse, w_regularizer=None, name='fc2')
  if use_bn:
    fc = tf.nn.relu(batch_norm(fc, is_training=is_training, reuse=reuse, name='fc2/bn'))
  else:
    fc = rtf.nn.elu(fc, name='fc2/prelu')
  out = tf.nn.sigmoid(dense(fc, isize, is_training, reuse, w_regularizer=None, name='g_out'))
  return out
...
def discriminator_from_params(x, params, is_training, reuse, isize=28 * 28, n_hid=100):
  fc = dense(x, n_hid, is_training, reuse, w_normalized=True, name='fc1', params=params[:2])
  fc = tf.nn.relu(fc, name='fc1/relu')
  fc = dense(fc, n_hid, is_training, reuse, w_normalized=True, name='fc2', params=params[2:4])
  fc = tf.nn.relu(fc, name='fc2/relu')
  ...
  with SessionWrap(session) as sess:
    ...
    for t in range(n_steps):
      for i in range(d_steps):
        z = z_gen()
        _, curr_loss_d = sess.run([opt_d, loss_d], feed_dict={feed_z: z})
      if t > d_pretrain_steps:
        for i in range(g_steps):
          z = z_gen()
          _, curr_loss_g = sess.run([opt_g, loss_g], feed_dict={feed_z: z})
      else:
        curr_loss_g = 0.
...
def main():
  ...
  with g.as_default():
    ...
    x_tf = tf.reshape(mnistdata.train_batch(batch_size)[0], shape=(-1, 28 * 28))
    ...
    with tf.variable_scope('D') as scope:
      disc_out = discriminator(tf.concat([x_tf, x_gen], 0), z_size, True, None)
    ...
    loss_d = tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(
            logits=tf.concat([disc_real, disc_fake], 0),
            labels=tf.concat(
                [tf.ones_like(disc_real), tf.zeros_like(disc_fake)], 0)))
    ...
    if lookahead > 0:
      for i in range(lookahead):
        ...
        loss_d_tmp = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                logits=tf.concat([disc_real_g, disc_fake_g], 0),
                labels=tf.concat([tf.ones_like(disc_real_g),
                                  tf.zeros_like(disc_fake_g)], 0)))

        grads = tf.gradients(loss_d_tmp, next_d_params)
        next_d_params, opt_vars = optimizer_d.unroll(grads, next_d_params, opt_vars=opt_vars)
    else:
      disc_out_g = discriminator_from_params(tf.concat([x_tf, x_gen], 0), next_d_params, True, None)
      disc_fake_g = disc_out_g[batch_size:, :]
    ...
    train(
        ...
        g_steps=g_steps,
        d_steps=d_steps,
        d_pretrain_steps=d_pretrain_steps,
        session=session,
        callbacks=[logging],
        coord=coord)
    coord.request_stop()
