...
def get_data(data_dir):
  ...
  Returns:   Train and test data as dictionaries mapping   label to
  list of examples.
  """
...
def resize_images(images, new_width, new_height):
  ...
  for i in range(images.shape[0]):
    resized_images[i, :, :] = imresize(
        images[i, :, :], [new_width, new_height], interp='bilinear', mode=None)
  return resized_images


def write_datafiles(directory,
                    write_file,
                    resize=True,
                    rotate=False,
                    original_size=105,
  ...
  images, labels, info = crawl_directory(
      directory, augment_with_rotations=rotate, first_label=first_label)
  ...
  if resize:
    ...
    data = {'images': resized_images, 'labels': labels_np, 'info': info}
    with tf.gfile.GFile(write_file, 'w') as f:
      pickle.dump(data, f)
  else:
    logging.info('Writing original sized data in boolean format.')
    data = {'images': images_np, 'labels': labels_np, 'info': info}
    with tf.gfile.GFile(write_file, 'w') as f:
      pickle.dump(data, f)
...
def preprocess_omniglot(repo_dir,
                        repo_location,
                        data_dir,
                        train_dir,
                        test_dir,
  ...
  Downloads the data from GitHub if it does not exist. Then load the
  images, augment with rotations if desired. Resize the images and write
