"""Modified version of Original code https://github.com/ischlag/tensorflow-
input-pipelines/blob/master/datasets/svhn.py."""
...
class svhn_data(object):
  """Downloads the SVHN dataset and creates an input pipeline ready to be fed
  into a model.
  ...
  def train_batch(self, batch_size, shuffle=False, standardizer=None):
    ...
    return self.__build_generic_data_tensor(
        images, targets, shuffle, batch_size=batch_size, standardizer=standardizer)
  ...
  def __build_generic_data_tensor(self,
                                  raw_images,
                                  raw_targets,
                                  shuffle,
                                  batch_size=32,
    ...
    if self.shuffle:
      self.queue = tf.RandomShuffleQueue(
          ...
          min_after_dequeue=self.batch_queue_capacity,
    ...
    images_batch, target_batch = tf.train.batch(
        [image, target], batch_size=batch_size, capacity=self.batch_queue_capacity)
    ...
    def enqueue(sess):
      ...
      while not self.coord.should_stop():
        ...
        sess.run(enqueue_op, feed_dict={image_input: curr_data, target_input: curr_target})
