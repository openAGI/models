...
@click.command()
@click.option(
    '--weights_from', default='', show_default=True, help="Path to the trained weights file")
@click.option('--image_filename', default='', show_default=True, help="Image to run predictions.")
def main(image_filename, weights_from):
  ...
  try:
    node_to_string = open('examples/classification/imagenet1000_clsid_to_human.txt',
                          'r').readlines()
  except Exception:
