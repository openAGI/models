...
class GetData(object):
  ...
  def next(self, batch_size, train=True):
    ...
    if self.batch_id + batch_size <= len(self.traindata):
      batch_data = (
          self.traindata[self.batch_id:min(self.batch_id + batch_size, len(self.traindata))])
      batch_labels = (
          self.trainlabels[self.batch_id:min(self.batch_id + batch_size, len(self.traindata))])
  ...
  def next_test(self, batch_size, train=True):
    ...
    if self.batch_id + batch_size <= len(self.testdata):
      batch_data = (self.testdata[self.batch_id_test:min(self.batch_id_test + batch_size,
                                                         len(self.testdata))])
      batch_labels = (self.testlabels[self.batch_id_test:min(self.batch_id_test + batch_size,
                                                             len(self.testdata))])
      self.batch_id_test = min(self.batch_id_test + batch_size, len(self.testdata))
    else:
...
def model(x, y, batch_size, is_training=True, reuse=None):
  with tf.variable_scope('model', reuse=reuse):
    ...
    conv1 = conv2d(h_trans, 16, is_training, reuse, activation=prelu, name='conv1')
    conv2 = conv2d(conv1, 16, is_training, reuse, stride=(2, 2), activation=prelu, name='conv2')
...
def main():
  ...
  accuracy, loss, optimizer = model(x, y, batch_size_train, is_training=True, reuse=None)
  n_epochs = 500
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
      ...
      for step in range(n_iters_per_epoch_test):
        ...
        epoch_acc += sess.run(accuracy, {x: batch_x_test, y: batch_y_test})
      epoch_acc = epoch_acc / n_iters_per_epoch_test
      print('Epoch {:2d} Accuracy {:3.1f}%'.format(epoch + 1, 100 * epoch_acc))
